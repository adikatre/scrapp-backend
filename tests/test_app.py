...
@patch("app.model")
def test_predict_success_with_openai_fields(mock_model, mock_classify, client):
...
@patch("app.classify_waste")
@patch("app.model")
def test_predict_fallback_preserves_legacy_fields(mock_model, mock_classify, client):
    mock_model.predict.return_value = _mock_yolo_result()
    mock_classify.return_value = ClassificationResult(
        items=[
            {
